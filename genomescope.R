...
option_list <- list(
  optparse::make_option(c("-i", "--input"), type="character",
    help="File with histogram data."),
    optparse::make_option(c("-k", "--kmer"), type="integer",
    help="K-mer length"),
    optparse::make_option(c("-l", "--readLength"), type="integer",
    help="Reads legnth."),
    optparse::make_option(c("-o", "--output"), type="character",
    help="Output file."),
    optparse::make_option(c("-K", "--kmerMax"), type="integer", default = -1,
    help="(Optional) Maximum kmer length."),
    optparse::make_option(c("-v", "--verbose"), type="logical",
    action = "store_true", default = FALSE,
    help="Activates verbose mode"),
    optparse::make_option(c("-e", "--epochs"), type="integer", default = 4,
    help="(Optional) Number of rounds before giving up. Default: %default"),
    optparse::make_option(c("-s", "--shift"), type="integer", default = 5,
    help=paste0("(Optional) Coverage steps to trim off between rounds. ",
        "Default: %default")),
    optparse::make_option(c("-E", "--errorthr"), type="integer", default = 15,
    help="(Optional) Typical cutoff for sequencing error. Default: %default"),
    optparse::make_option(c("-n", "--nls"), type="integer", default = 20,
    help="(Optional) Max rounds on NLS. Default: %default"),
    optparse::make_option(c("-c", "--closeScore"), type="double", default = 0.2,
    help=paste0("(Optional) Overrule if two scores are within this percent ",
        "(0.05 = 5%) but larger difference in het. Default: %default")),
    optparse::make_option(c("-H", "--hfd"), type="integer", default = 10,
    help=paste0("(Optional) Overrule heterozygosity if there is a large ",
        "difference in het rate. Default: %default")))
...
foldername <- opt$output 
maxCovGenomeLen <- opt$kmerMax
VERBOSE <- opt$verbose
